...
use crate::engine::renderer::{rgb_fg, render_usage_bar, RESET};
...
impl UsageSegment {
    pub fn render(&self, _input: &InputData, theme: &ResolvedTheme) -> Vec<SegmentOutput> {
        ...
        let current_bar = render_usage_bar(
            data.five_hour_utilization, 10,
            &theme.usage_current_fill, &theme.usage_empty,
        );
        ...
        let current_line = format!(
            ...
            current_bar, RESET, current_pct, current_reset
        );

        let weekly_bar = render_usage_bar(
            data.seven_day_utilization, 10,
            &theme.usage_weekly_fill, &theme.usage_empty,
        );
        ...
        let weekly_line = format!(
            ...
            weekly_bar, RESET, weekly_pct, weekly_reset
        );
...
fn format_reset_time(resets_at: &Option<String>, include_date: bool, theme: &ResolvedTheme) -> String {
...
fn cache_path() -> PathBuf {
    ...
    home.join(".claude").join("statusline-rs").join("usage_cache.json")
}
