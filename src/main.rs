...
fn main() {
    ...
    if enabled.contains(&"context_window".into()) {
        if let Some(out) = segments::context_window::ContextWindowSegment.render(&input, &theme) {
            line1_parts.push(out.text);
        }
    }
    ...
    if enabled.contains(&"effort".into()) {
        if let Some(out) = segments::effort::EffortSegment.render(&input, &theme) {
            line1_parts.push(out.text);
        }
    }

    // Line 1
    let line1 = join_segments(&line1_parts, &config.general.separator, theme.separator_ansi);
    println!("{}{}", line1, RESET);
    ...
    // Line 4: Tasks
    if enabled.contains(&"tasks".into()) {
        if let Some(out) = segments::tasks::TasksSegment.render(&input, &theme) {
            println!("{}{}", out.text, RESET);
        }
    }
}
