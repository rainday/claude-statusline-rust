...
pub struct ResolvedTheme {
    ...
    pub effort_high_ansi: u8,      // bright magenta (13)
...
impl ResolvedTheme {
    pub fn morandi() -> Self {
        Self {
            ...
            effort_high_ansi: 13,    // bright magenta
    ...
    pub fn from_config(_name: &str, overrides: &Option<ThemeColors>) -> Self {
        ...
        if let Some(o) = overrides {
            macro_rules! apply {
                ($field:ident) => {
                    if let Some(ref c) = o.$field { theme.$field = c.clone(); }
                };
            }
            apply!(directory); apply!(git); apply!(model);
            apply!(ctx_green); apply!(ctx_yellow); apply!(ctx_orange); apply!(ctx_red);
            apply!(thinking_on); apply!(thinking_off);
            apply!(effort_high); apply!(effort_dim);
            apply!(usage_current_fill); apply!(usage_weekly_fill); apply!(usage_empty);
            apply!(reset_time); apply!(tasks); apply!(separator);
        }
        theme
    }
}
